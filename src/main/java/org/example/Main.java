...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            logger.info("  GET    /                    - Web management UI / List all buckets");
            logger.info("  GET    /{bucket}            - List objects in bucket");
            logger.info("  GET    /{bucket}/{key}      - Download object");
            logger.info("  PUT    /{bucket}            - Create bucket");
            logger.info("  PUT    /{bucket}/{key}      - Upload object");
            logger.info("  DELETE /{bucket}            - Delete bucket");
            logger.info("  DELETE /{bucket}/{key}      - Delete object");
            logger.info("========================================");
            logger.info("Example commands:");
            logger.info("  # Create a bucket");
            logger.info("  curl -X PUT http://localhost:{}/my-bucket", port);
            logger.info("");
            logger.info("  # Upload a file");
            logger.info("  curl -X PUT --data-binary @myfile.txt http://localhost:{}/my-bucket/myfile.txt", port);
            logger.info("");
            logger.info("  # Download a file");
            logger.info("  curl http://localhost:{}/my-bucket/myfile.txt", port);
            logger.info("  # List objects in bucket");
            logger.info("  curl http://localhost:{}/my-bucket", port);
            logger.info("  # Delete a file");
            logger.info("  curl -X DELETE http://localhost:{}/my-bucket/myfile.txt", port);
            logger.info("========================================");
